...
const itemData = allItems.map(({ id, name, price, itemHasBeenPurchased }) => (
  ...
  <div className="container">
    <h3 className="green-text center-align">Lista de mercado App</h3>
    <form className="add-item-form" onSubmit={event => addItemToDb(event)} >
      <input type="text" className="item-name" placeholder="Nombre del producto" required/>
      <input type="number" step=".01" className="item-price" placeholder="Precio" required/>
      
      <button type="submit" className="waves-effect waves-light btn right">Agregar producto</button>
    </form>
    {allItems.length > 0 &&
